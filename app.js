// app.js - Met Bulk Upload Functionaliteit
...
async function handleFileUpload(e, isBulk) {
    ...
    // Als het maar 1 bestand is, doen we de "normale" visualisatie
    if (files.length === 1) {
        const file = files[0];
        if(!file.name.toLowerCase().endsWith('.gpx') && !file.name.toLowerCase().endsWith('.tcx')) return;
        
        const text = await file.text();
        // Visualiseer direct
        processGPXAndRender(text, file.name);
    } 
    // Als het meerdere bestanden zijn (Bulk/Map)
    else {
...
async function processBulkUpload(files) {
    ...
    if(total === 0) {
        statusDiv.innerHTML = "Geen GPX bestanden gevonden in map.";
        return;
    }

    statusDiv.innerHTML = `⏳ Bezig met 0 van ${total} ritten...`;

    // Loop er doorheen (Sequential om browser niet te laten crashen)
    for (let i = 0; i < total; i++) {
        const file = validFiles[i];
        
        try {
            statusDiv.innerHTML = `⏳ Verwerken: ${file.name} (${i+1}/${total})`;
            
            const text = await file.text();
            
            // 1. Parse data (zonder renderen)
            const rideData = parseGPXData(text, file.name);
            
            if (rideData) {
                // 2. Direct opslaan in Supabase
    ...
    // Ververs dashboard en ranglijst cache
    if(window.allActivitiesCache) window.allActivitiesCache = null;
    if(window.updateDashboard) window.updateDashboard();
    
    alert(`Batch upload klaar! ${successCount} ritten toegevoegd.`);
}

// --- SINGLE RIT (Renderen + Klaarzetten voor save) ---
function processGPXAndRender(xmlString, fileName, isExistingRide = false) {
    ...
    // 4. Zet globaal voor de opslaan-knop
    currentRideData = data;
}

// --- CORE: PARSING LOGICA (GESCHEIDEN VAN UI) ---
function parseGPXData(xmlString, fileName, isExistingRide = false) {
    ...
    return {
        ...
        // UI data (niet nodig voor opslaan, wel voor renderen)
        uiData: { latlngs, elevations, distances, durationMs }
    };
}

// --- REST VAN DE FUNCTIES (Helpers, UI Updates) ---
...
window.openRide = async function(activity) {
    ...
    } catch (e) { console.error(e); alert("Kon rit niet openen."); }
};
...
async function saveToCloud() {
    ...
    try {
        ...
        alert(e.message); btn.innerText = "Opslaan"; btn.disabled = false;
    }
}
